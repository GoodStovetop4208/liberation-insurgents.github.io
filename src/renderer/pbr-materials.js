const PBRMaterials = (() => {
    ...
    function configure(texture, colorTexture = false, repeatX = 3, repeatY = 3) {
        ...
        if (colorTexture && THREE.sRGBEncoding) texture.encoding = THREE.sRGBEncoding;
    ...
    function load(path, colorTexture = false, repeatX = 3, repeatY = 3) {
        return new Promise(resolve => {
            loader.load(basePath + path, texture => resolve(configure(texture, colorTexture, repeatX, repeatY)), undefined, () => resolve(null));
        });
    }
    ...
    function create(kind = 'concrete', color = 0xffffff, repeatX = 3, repeatY = 3) {
        ...
        const material = new THREE.MeshPhysicalMaterial({
            color,
            roughness: isSand ? 0.96 : 0.68,
            metalness: 0,
            clearcoat: isSand ? 0 : 0.12,
            clearcoatRoughness: isSand ? 1 : 0.55,
            envMapIntensity: isSand ? 0 : 0.75,
            reflectivity: isSand ? 0.02 : 0.5,
